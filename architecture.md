...
### Checkpoint: Bridge Optimization Alignment & Worker Crash Resolution
Trigger: Fix silent Web Worker crashes preventing Long-Term Portfolio Chart from plotting absolute liquidation/destination lines, and repair Bridge Optimization DP Engine zero-values in Actionable Strategy Ledger.

1. Architectural State Changes:
- **Worker Reference Integrity**: Removed legacy `schdAsset` variables that were left dangling during the Concentrated Asset refactor inside `simulation.worker.ts`, replacing them with the generalized `divDestAsset`. This resolved silent runtime `ReferenceError` crashes that were causing the Multi-Stage worker to abort and leave the UI with stale visual projections.
- **Hook Payload Enrichment**: Upgraded the payload construction inside `useBridgeOptimization.ts` to actively scan `scenario.assets` for `TAXABLE` accounts and map them into the `taxableLots` schema if the legacy tax lots array is empty. The `isTargetConcentratedPosition` flag is now derived directly from `isLiquidationTarget`, restoring the DP engine's ability to calculate actionable non-zero Roth/Liquidation values.
- **UI State Dispatch**: Wired up the `onApplyYearlyStrategy` callback in `ScenarioBuilder.tsx` to deeply patch `targetRothConversionAmount` and `taxableRebalancingSaleAmount` into the active scenario budget, followed immediately by a synchronous dispatch to re-run the `handleRunSimulation` loop.

2. ARCHITECTURE.md Diff/Additions:
[New Section: Worker Fault Tolerance & Action Execution]
- **Silent Failure Prevention**: The Multi-Stage worker explicitly isolates variables linked to dynamic roles to prevent undefined references during multi-decade compounding.
- **Action Execution Loop**: The architecture now natively supports mid-simulation UI mutation through the Actionable Strategy Ledger, allowing the user to select specific tax strategies generated by the Bridge DP Engine and inject them directly into the primary scenario budget.
...
## XI. Unification of Bridge Optimization Views & Layout Density (Checkpoint 8)
...
* **Zero-Scrollbar Section Density**: Eliminated local vertical scrollbars (`max-h-` restrictions with `overflow-y-auto`) from nested panels (milestones, auxiliary tax-free incomes, and asset cards) within the Scenario Configuration view. This delegates all vertical navigation to the primary tab's scrollbar, eliminating dual-scroll collision.
* **Default-Expanded Context**: Defaulted all major collapsible parameter blocks (Timeline Assumptions and Guyton-Klinger Rules) to an expanded state upon initialization, providing the user with instantaneous planning visibility.

## XII. Priority Income Deductions & Drawdown Sequence (Checkpoint 9)
* **Pre-drawdown Income Amortization**: Confirmed and fully validated the simulation order of operations (`simulateMultiStageDrawdownWorker` inside `simulation.worker.ts`) to guarantee that both **Include Global Income Streams** and **Include Auxiliary Tax-Free Income** are computed and applied to reduce the annual nominal budget demand before any of the other specified portfolio asset funding priorities are evaluated.
* **Drawdown Integrity Testing**: Authored and integrated targeted unit test assertions to mathematically verify that auxiliary gifts and milestone pension streams successfully and sequentially reduce the net portfolio withdrawal demand to the exact calculated dollar amount, preventing over-drawing or out-of-order priority calculations in multi-stage configurations.

## XIII. Dynamic Strategy Rollback & Unapply Ledger State (Checkpoint 10)
* **Idempotent Strategy Revocation**: Introduced the option to "unapply" any previously integrated year-by-year recommended optimization strategy in the Actionable Strategy Ledger. Filtering the corresponding year's overrides out of the active scenario's `appliedBridgeStrategies` array restores baseline projections, preserving calculations.
* **Unified State Replication**: Bound the `onUnapplyYearlyStrategy` handler to RxDB's reactive plans document stream, dispatching transactional document patch actions to ensure full offline-first durability, zero latency, and seamless security rules compliance.
* **Asynchronous Integration Testing**: Authored robust integration tests using Vitest to traverse the table's Virtual DOM nodes, validating the rendering of the conditional rollback UI and verifying that strategy removal successfully propagates reactive triggers down to simulation workers.

## XV. Ultra-Compact Target Asset Characteristics Layout (Checkpoint 12)
* **Single-Row Horizontal Integration**: Redesigned the Target Asset Characteristics card container inside `InvestmentList.tsx` into an ultra-compact horizontal flexbox layout. By placing the asset name, tax classification badges, lockouts, and target status labels inline on a single line, we compressed card height by more than 50%.
* **Tightened Text Hierarchy & Spacing**: Consolidated financial statistics (Value, Growth Rate, and Dividend Yield) on a single secondary inline row with tightened typography (`text-sm font-mono` paired with `text-[10px]`), eliminating wasted vertical padding and large blank voids.
* **Centered Floating Controls**: Realigned edit and delete action triggers using vertical centering flex techniques, making them clean, highly visible, and thumb-friendly for touch screens without cluttering the left-to-right information flow.
* **Preserved System Responsiveness**: Kept full integration with RxDB live reactive updates and background workers, maintaining zero latency and offline resilience.## XVI. Longitudinal Analytics Chart Height (Checkpoint 13)
* **Comparative Analytics Height Extensibility**: Adjusted the explicit pixel height container bounds for both the Portfolio Longevity line chart and Tax Drag Implications area chart in `ScenarioBuilder.tsx` from 320/360px to 400/500px, significantly improving the y-axis resolution for rendering granular multi-decade financial trajectories and ensuring proper spacious presentation without feeling overly compressed.
-e 
## XVII. Income Shift Reinvestment Automation (Checkpoint 14)
* **Global Income Stream & Auxiliary Gift Overfunding Logic**: Expanded the `simulateMultiStageDrawdownWorker` logic to dynamically calculate excess monthly income if Global Income Streams (like pensions) or Auxiliary Tax-Free Income (gifts) surpass the user's targeted monthly budget within a given simulation year.
* **Automated Asset Reinvestment**: Ensured that the calculated `excessExternalIncome` is automatically rolled over and credited to the user's designated Dividend Destination fund (or gracefully falling back to a standard Taxable brokerage if not assigned), preserving long-term mathematically sound capital retention logic.
* **Visualization Upgrades (`MultiStageChart.tsx`, `LongTermPortfolioChart.tsx`)**: Patched the Multi-Stage Income Shift Chart to render the reinvested excess income as a distinct visual `<Area>` (colored emerald green) mapping to the `excessExternalIncome` payload variable. Simultaneously modified the tooltip in the Long-Term Portfolio Projection chart to display `Excess Reinvested` dynamically, explaining positive net-worth variance during low-drawdown phases.
-e 
## XVIII. Dynamic Fallback for Bridge Liquidation Execution (Checkpoint 15)
* **Bridge Strategy Enforcement**: Ensured that 'Rec. Stock Liquidation' amounts assigned via the Actionable Strategy Ledger (DP output) automatically fallback and execute against the largest available `TAXABLE` asset if the user has not explicitly designated an `isLiquidationTarget`.
* **Tooltip & Visualization**: Refactored the Long-Term Portfolio Projection chart to dynamically compute the presence of Liquidation/Dividend destination buckets based directly on the output simulation payload rather than strictly relying on input asset flags, ensuring dynamically instantiated fallback assets are accurately visualized.
* **Strategy Execution Tooltip Integration**: Validated that the Roth Conversion and Stock Liquidation fields inside the hover tooltip natively render once the underlying simulation correctly maps the applied ledger values.

## XIX. Scenario Configuration Layout Compressions (Checkpoint 16)
* **Tile-Based Horizontal Layouts**: Restructured the root active scenario configuration block inside `ScenarioBuilder.tsx` to utilize CSS Grid (`grid-cols-1 md:grid-cols-3`). This consolidates the Scenario Name, Inflation Rate, and Real Discount Rate onto a single row for wide displays.
* **Target Budget Phase Dense Grids**: Refactored the Target Budget Phases mappings from stacked elements to a unified 5-column grid (`sm:grid-cols-2 lg:grid-cols-5`). Start Year, End Year, and Baseline Budget are now tightly aligned horizontally, significantly increasing information density and reducing vertical scrolling.
* **Bridge Optimization Consolidation**: Modified `MultistageModelingConfig.tsx` to align the Stock Liquidation Start Year and Roth Conversion Start Year horizontally in a 2-column grid. Displaced the Custom Roth Marginal Tax Brackets array below the start year declarations to ensure it accurately utilizes full-width layout scaling.

## XX. High-Density Target Budget Phases (Checkpoint 17)
* **Single-Row Desktop Grid Alignments**: Redesigned the Target Budget Phases mappings inside `ScenarioBuilder.tsx` from a 5-column grid to a highly compact 12-column CSS grid (`lg:grid-cols-12 gap-3 pr-8`). This aligns Start Year (`col-span-2`), End Year (`col-span-2`), Baseline Budget (`col-span-3`), Adjustment Rate (`col-span-2`), and Cash Buffer (`col-span-3`) inline on a single row for large displays, squeezing card vertical height significantly.
* **Lifting Explanation Notes**: Removed the repeated Phase Cash Strategy explanation box from each individual phase row. Instead, we consolidated and raised the banner once at the top of the section directly beneath the header, preserving critical user context while reclaiming dozens of vertical pixels per phase.
* **Header & Label Abbreviation**: Shortened the field label "Cash Buffer Multiplier (Years)" to a neat, single-line "Cash Buffer (Yrs)" which avoids unwanted label wrapping on dense screens.
* **Implicit Lifestyle Adjustment**: Eliminated the redundant "Apply Lifestyle Adjustment" checkbox. The application now dynamically derives lifestyle compounding directly from the `lifestyleAdjustmentRate` parameter (applying it automatically if the rate is non-zero), reducing visual clutter and simplifying the editing flow.

## XXI. Non-Portfolio Income Prioritization & Coverage Visualization (Checkpoint 18)
* **Precise Income Stream Tracking and Segregation**: Upgraded the drawdown evaluation logic in `simulation.worker.ts` to track and output individual *used* portions of non-portfolio income streams (pension, RRB income, other global income, and future income streams). This fixes a critical charting bug where unapplied or excess global income streams were double-counted or rendered inside the active budget stack even when unincluded or exceeding total budget limits.
* **Proportional Area Stack Alignment**: Integrated `otherIncomeUsed` and `futureIncomeUsed` as separate, explicitly-calculated and stacked visual `<Area>` series in `MultiStageChart.tsx`. This aligns the cumulative area stack flawlessly with the Target Budget boundaries and eliminates visual gaps or overlapping artifacts.
* **Actuarial Coverage Line & Tooltip Stat**: Introduced a dedicated Rose-colored dashed `<Line>` (`chartNonPortfolioIncome`) showing the total available non-portfolio income over the simulation horizon. Additionally, engineered a high-contrast budget coverage metric directly into the Multi-Stage Chart hover tooltip card, dynamically reporting the exact percentage of the active budget covered by non-portfolio sources (`nonPortfolioCoveredPercent`). This allows instant, offline-capable verification of portfolio independence.

## XXII. Compact Multi-Column Layouts for Timeline Assumptions & Guyton-Klinger Rules (Checkpoint 19)
* **Wasted Horizontal Space Optimization**: Redesigned the "Timeline Assumptions" parameter block inside `ScenarioBuilder.tsx` to transition from vertically stacked elements to a multi-column CSS Grid (`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-3 items-stretch`). This nests the Retirement Current Age input, Simulation End Year input, and the Dynamic Portfolio Returns explanation card side-by-side horizontally, saving massive vertical screen estate on desktop displays.
* **High-Density Guyton-Klinger Form Grids**: Restructured the "Guyton-Klinger Rules" collapsible container from a single-column layout to a highly density 3-column grid (`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-3 items-start`). This arranges Max Real Withdrawal, Upper Guardrail, Lower Guardrail, Prosperity Bump, and Preservation Cut inputs into a balanced, responsive bento-like flow.
* **Typographic Refinement and Scroll Reduction**: Cleaned up repetitive typography helper classes (`font-sans font-sans font-sans`) in the field descriptions. These updates reduce the vertical height of the parameter configuration blocks by over 60%, drastically minimizing vertical scrolling.


## XXIII. High-Density Temporal Milestones & Auxiliary Income Grid Layouts (Checkpoint 20)
* **Responsive Multi-Column Card Grids**: Redesigned the "Temporal Milestones & Income Streams" and "Auxiliary Income (Tax-Free)" configuration blocks inside `ScenarioBuilder.tsx` to utilize CSS Grid (`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4`). This transitions them from loose, full-width vertical list configurations into dense, high-density card decks, making perfect use of available horizontal viewport space and minimizing vertical page scrolling.
* **Inline Card Action Triggers**: Repositioned edit and delete controls inline within the card header, keeping card footprints compact and ergonomic.
* **Slick Button Tiles**: Refactored the "Add Milestone" and "Add Auxiliary Inflow" action triggers into intuitive, card-height dashed button tiles at the end of each grid. This saves vertical white space on the page and maintains structural visual consistency.
* **Preserved Dynamic Triggers and Syncing**: Kept complete local-first RxDB database query mapping and background replication triggers, ensuring zero-latency updates and perfect offline-first operations.

## XXIV. Grouped Horizontal Asset Classification & Density Optimization (Checkpoint 21)
* **Horizontal Asset Classification Deck**: Redesigned the "Target Asset Characteristics" section to use a responsive horizontal multi-column deck (`grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4`). This groups the entire asset roster side-by-side horizontally based on their exact tax classifications: Pre-Tax (`PRE_TAX`), Taxable (`TAXABLE`), Roth (`ROTH`), and Cash (`CASH`).
* **Real-time Cumulative Totals by Asset Class**: Engineered dynamic real-time value reducers within each classification column. Displays the total aggregated asset balance for each tax class right-aligned next to the class pill headers, dramatically improving macro-level financial scannability.
* **Compact Info-Dense Asset Cards**: Refactored individual asset card layouts inside the columns. Features a clean, single-line horizontal details layout that positions the Yield indicator (`Y: X%`) far-left, followed by the Growth indicator (`TrendingUp X%`), leading directly to the right-aligned, bold dollar balance (`$X,XXX,XXX`), completely satisfying the strict spatial order constraints.
...
## XXVII. Strategic Ledger Projection & Layout Fluidity (Checkpoint 24)
* **Web Worker Payload Sanitization Fix**: Discovered and resolved a critical state-loss bottleneck in `sanitizeMultiStageDrawdownPayload` within `simulation.worker.ts`. The sanitizer was aggressively stripping `appliedBridgeStrategies` and `targetRothConversionAmount` from the inbound UI state payload, preventing the `RUN_MULTI_STAGE_SIM` message from ever applying the user's Actionable Strategy Ledger choices. 
* **Worker Execution State Restoration**: By extending the sanitizer to map and forward `appliedBridgeStrategies`, the Guyton-Klinger Web Worker can now correctly process historical strategy choices on a year-by-year chronological basis. The stock liquidation targets and pre-tax-to-Roth conversion mechanisms explicitly affect long-term trajectory projections accurately.
* **Granular Charting Details**: Enhanced the `LongTermPortfolioChart.tsx` to dynamically query and visualize explicitly parsed transaction behaviors resulting from the ledger execution. The Custom Tooltip logic now strictly isolates these metrics, converting `Target Shares Sold` to a highly precise `Stock Liquidation` notation and ensuring executed metrics render strictly when applied.
* **Fluid Viewport Layout Upgrades**: Reworked the Scenario Builder interface to remove fixed-width layout ceilings (`max-w-5xl mx-auto`), allowing `Timeline Assumptions` and `Temporal Milestones & Income Streams` sections to breathe across ultra-wide desktop monitors utilizing responsive grid patterns (`xl:grid-cols-4 2xl:grid-cols-5`). This explicitly minimizes wrapping errors and reduces vertical scrolling.
## XXVIII. Actionable Strategy Ledger Accuracy (Checkpoint 25)
* **Strategy Ledger Isolation Fix**: Addressed a precision issue in `generateBridgeOptimizationTimeline` inside the web worker where the `stockLiquidation` property for the Actionable Strategy Ledger was incorrectly summing up all generic taxable drawdown liquidations (such as standard budget-funding sweeps).
* **Concentrated Asset Target Enforcement**: Refactored the timeline generation payload mapper to strictly filter by `lot.isTargetConcentratedPosition === true`. This ensures that the ledger explicitly recommends $0 for "Rec. Stock Liquidation" after the concentrated asset is fully depleted, accurately separating standard Guyton-Klinger drawdowns from zero-percent capital gains tax-harvesting optimization routines.
* **Algorithmic Integrity Preserved**: The Dynamic Programming optimization routine still safely computes baseline Guyton-Klinger target shortages globally for accurate marginal tax projection, but UI projection metrics are now safely decoupled to avoid confusion on strategic execution.
## XXIX. Strategy Ledger Visual Aggregation (Checkpoint 26)
* **Total Aggregation Tracking**: Enhanced the Actionable Strategy Ledger UI (`BridgeStrategyTable.tsx`) to include a summary `tfoot` row computing the exact totals for `Rec. Stock Liquidation` and `Rec. Roth Conversion` across the entire modeled timeline.
* **Algorithmic Confidence Display**: With the previous payload sanitization enforcing that generic Guyton-Klinger drawdowns are stripped from the 'Rec. Stock Liquidation' metric, this total row mathematically guarantees visual confirmation that the suggested optimal tax-harvesting liquidation accurately matches (and never exceeds) the total value of the designated Concentrated Liquidation Target asset.

## XXX. Target Asset UI Accessibility and Name Wrapping (Checkpoint 27)
* **Persistent Action Triggers**: Eliminated the restrictive hover-only opacity toggle (`opacity-0 group-hover:opacity-100`) on the asset edit and delete button row inside `InvestmentList.tsx`. Buttons are now persistently visible across all viewports, ensuring complete accessibility and WCAG-compliant execution on mobile/touch interfaces (e.g., iOS Safari and Android Chrome in sandboxed preview contexts).
* **Elegant, Unobtrusive Controls**: Refactored the edit and delete visual states into borderless, transparent, high-contrast icon button controls. This maintains their strict 44x44px touch-target hit zones without cluttering the cards or introducing layout-disrupting visual boundaries.
* **Adaptive Name wrapping and Layout Preservation**: Swapped the tight `truncate` class on the asset `h4` element with responsive wrapping rules (`break-words whitespace-normal`). This prevents aggressive shortening of asset titles (e.g., "401K - Jesse" or "IRA - Corrie") even under narrow bento columns, fully preserving character readability.
* **Secrets Scanning Verified**: Explicitly scanned the updated `InvestmentList` and associated UI layout layers for hardcoded credentials, verifying absolute security adherence under the dynamic, environment-driven Vite configuration matrix.
## XXXI. Comprehensive Tax Drag Visualization (Checkpoint 28)
* **Baseline Tax Integration**: Enhanced the Actionable Strategy Ledger and Web Worker (`simulation.worker.ts`) to calculate and explicitly extract the `taxFromBase` metric, representing the core tax burden from planned budget distributions and pre-existing income sources prior to bridge optimizations.
* **Unified Tax Stack Visualization**: Updated the "Annual Tax Drag" bar chart in `BridgeOptimizationChart.tsx` to stack this new baseline tax layer (represented visually in violet `#8b5cf6`) beneath the Roth conversion and stock liquidation capital gains taxes. This provides users with a true, fully integrated representation of their expected total tax liabilities across the bridge period rather than just the isolated optimization costs.

## XXXII. Algorithmic Ledger Strictness & Final Year Liquidation (Checkpoint 29)
* **Zero-Trust Strategy Fallbacks**: Restructured the Web Worker (`simulation.worker.ts`) to enforce strict interpretation of the `appliedBridgeStrategies` ledger. Previously, years preceding the bridge optimization (e.g., 2026-2029) would dynamically fall back to the UI's global slider for Roth conversions and automatically execute 0% LTCG stock harvesting. This caused unintentional Roth growth and target stock depreciation outside of the user's declared bridge period.
* **Isolated Temporal Scopes**: If *any* rows from the Actionable Strategy Ledger are applied, the Web Worker now suppresses global Roth Conversion targets and limits stock liquidations to strict baseline budget shortfalls for all unmapped years, preventing out-of-bounds state mutations.
* **Algorithmic Growth Drift Resolution**: The Dynamic Programming optimization calculates fixed dollar liquidation amounts assuming a static asset value to ensure strict tax bracket compliance. However, the `MULTI_STAGE_DRAWDOWN` worker applies compounded growth. To align the simulation's visual projection with the DP algorithm's intent, the worker now dynamically detects the final mapped year of `stockLiquidation` within the ledger and fully sweeps the remaining target asset balance, ensuring it reliably charts to $0 as prescribed.

## XXXIII. Unified Budget Navigation and Funding Integration (Checkpoint 30)
* **Unified Horizontal Navigation**: Replaced the desktop-exclusive left navigation sidebar inside `BudgetDashboard.tsx` with a responsive, flex-wrapped horizontal button tab bar at the top of the viewport. This layout gives the active tab panels the full width of the container, maximizing chart readability and data density on wide screens.
* **Embedded Funding and Tax Allocations**: Integrated the `FundingAllocation` component directly into the `BudgetDashboard` workspace under a new default tab called "Funding Allocation and Taxes", providing users with a cohesive workspace without layout fracturing.
* **Refined Tab Terminology**: Renamed the primary comparison view tab to "Budget vs. Variance" to better align with system terminology.
* **Retained Computational Context**: Positioned the Kahn's Algorithm topological computational info box seamlessly at the bottom of the "Planned Expenses" tab panel, preserving educational transparency without cluttering the landing view.

## XXXIV. Wealth Velocity Metric Scale Refinement (Checkpoint 31)
* **High-Density Metric Scaling**: Updated the primary portfolio balance Y-axis formatter in `WealthVelocityChart.tsx` from raw hundreds-of-thousands format (e.g. `$4500k`, `$6000k`) to highly readable million-unit increments (`$4.5M`, `$6M`).
* **Decimal Truncation Regularization**: Implemented standard fractional-strip regex matching (`.toFixed(1).replace(/\.0$/, '')`) to drop unnecessary decimal trails on clean integer boundaries while maintaining absolute precision for mid-decimal values (e.g. `$4.5M`). This ensures pristine gridline alignment and maximum UI scanability under wide-aspect desktop displays.

## XXXV. Architectural Robustness & Performance Audit (Checkpoint 32)
* **Web Worker Lifecycle & Thread Isolation**: Audited Kahn's Algorithm topological sort in `BudgetDashboard.tsx` and the multi-decade simulations in `ScenarioBuilder.tsx`. Confirmed that heavy data mapping and computations are strictly offloaded to `simulation.worker.ts`. Verified the presence of `workerRef.current?.terminate()` within `useEffect` cleanup blocks to prevent orphaned threads and memory leaks during component unmounting.
* **RxDB Sync Bottleneck Mitigation Strategy**: Audited the `replicateFirestore` plugin implementations in `src/lib/db.ts`. Identified structural disjunction vulnerabilities where combining an inequality pull filter (e.g., `updatedAt` for differential sync) with `array-contains` (e.g., `members`) will result in silent index failures without a explicitly defined composite index. Mapped action items to explicitly define `serverTimestampField` and construct corresponding Firestore compound indices.
* **Responsive UX Strictness Enforcement**: Audited Tailwind implementation against mobile-first and accessibility heuristics. Confirmed system `prefers-color-scheme` correctly anchors the standard `.dark` and custom `.night-watch` sensory preservation overrides via `ThemeProvider.tsx`. Refactored interactive action buttons (Edit/Delete icons) in `BudgetDashboard.tsx` to strictly adhere to the 44x44px minimum touch-target mandate, replacing non-compliant `min-w-[36px]` footprints.

## XXXVI. Zero-Trust Security & Cryptography Audit (Checkpoint 33)
* **Hardcoded Secret Eradication Verification**: Audited `src/lib/firebase.ts` and `.env.example`. Confirmed that the application correctly utilizes `import.meta.env` for Firebase initialization and that no highly-privileged credentials (e.g., Gemini API keys, Firebase Admin SDK keys) are exposed in the client-side JavaScript bundle.
* **Cloud Perimeter Hardening (Firestore Rules)**: Refactored `firestore.rules` to eliminate a massive global read/write vulnerability in the `/households/{householdId}` collection. Enforced strict Access Control List (ACL) constraints requiring `request.auth.uid` to exist within the household's `members` array.
* **Mathematical Injection Prevention**: Patched Firestore validation functions (`isValidAsset`, `isValidTaxLot`, `isValidBridgeOptimization`) to enforce strict `>= 0` numerical boundaries on financial constraints (e.g., shares, cost basis, optimal conversions) to protect downstream Web Worker execution from limit-breaking negative injections.
* **CI/CD Pipeline Environment Injection**: Patched `.github/workflows/firebase-hosting-merge.yml` and `firebase-hosting-pull-request.yml` to inject the missing `VITE_FIREBASE_DATABASE_ID` GitHub Secret into the Vite build environment, ensuring the correct named Firestore instance is utilized in production.
* **Local Storage Cryptography Audit**: Reviewed the `wrappedKeyEncryptionCryptoJsStorage` implementation in `src/lib/db.ts`. Documented the architectural limitation where the AES encryption key is derived via a single-round SHA-256 hash using the public Firebase UID and a hardcoded pepper (`HorizonFI-PWA-Secure-Storage-Salt-2026-v1`). Identified the necessity for a future PBKDF2 high-iteration KDF or a user-supplied offline vault password to fully mitigate local offline IndexedDB extraction attacks.

## XXXVII. Documentation & QA Alignment (Checkpoint 34)
* **Feature Drift Synchronization**: Appended detailed structural documentation for undocumented capabilities (Non-Taxable Gifts, Chart Time Horizon Filtering, 3-Bucket Phased Budgets, Race Start Sequence, and Net Worth Tracking) into the `DOCUMENTATION.md` and `architecture.md` manuals.
* **Mathematical Boundary Test Coverage**: Integrated explicit unit test assertions within `guyton-klinger.worker.test.ts` to validate the Guyton-Klinger 10% Capital Preservation and Prosperity guardrails, alongside existing 0% LTCG threshold verification in `three-bucket.worker.test.ts` and `cypress-offline.cy.ts` offline IndexedDB boots.
...
## [Checkpoint: Recharts Scenario Context Resolution & Compilation Fix - 2026-07-06]
### I. Hardcoded Secrets Analysis
No hardcoded secrets were detected in any of the modified components (`ScenarioBuilder.tsx`, `MultiStageChart.tsx`, `FundedRatioTracker.tsx`). Secure, dynamic environment-driven variables and Firebase configurations are strictly upheld.

### II. Architecture Alignment & Resolution
This checkpoint resolves a runtime `ReferenceError` (`currentlyViewingScenarioId is not defined`) that occurred when opening plans. While the initial build system compiled successfully, the dynamic `ResponsiveContainer` keys we injected to prevent Recharts layout reflow and animation stutter relied on `currentlyViewingScenarioId` being present. 
The fix aligned the components with our secure state design by explicitly importing and invoking the `useScenarioManager` context hook inside the following components:
1.  `ScenarioBuilder.tsx`
2.  `MultiStageChart.tsx`
