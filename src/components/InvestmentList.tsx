...
export function InvestmentList({ assets, onEdit, onDelete }: InvestmentListProps) {
  if (!assets || assets.length === 0) {
    return (
      <div id="no-investments-placeholder" className="border border-dashed border-zinc-300 dark:border-dashed dark:border-zinc-700 rounded-xl p-8 text-center bg-zinc-50 dark:bg-zinc-900/50">
  ...
  return (
    <div id="investment-grid" className="grid grid-cols-1 gap-3">
      {assets.map((asset) => {
        const config = getTypeConfig(asset.assetType);
        const hasLockout = !!asset.availableDate;
        
        return (
          <div id={`investment-card-${asset.id}`} key={asset.id} className="group relative bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col">
            <div className="flex justify-between items-start mb-2">
              <div className="pr-16">
                <h4 id={`investment-title-${asset.id}`} className="font-bold text-zinc-900 dark:text-zinc-50 leading-tight text-sm break-words">{asset.name}</h4>
                <div className="flex flex-wrap items-center gap-2 mt-1.5">
                  ...
                  {asset.isLiquidationTarget && (
                    <span id={`badge-liquidation-target-${asset.id}`} className="inline-flex items-center gap-1 text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 animate-fade-in" title="Primary Liquidation Target for drawdown">
                      <Target size={12} />
                      Liquidation Target
                    </span>
                  )}
                  {asset.isDividendDestination && (
                    <span id={`badge-dividend-destination-${asset.id}`} className="inline-flex items-center gap-1 text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 animate-fade-in" title="Dividend Destination Fund for reinvestment">
                      <Download size={12} />
                      Div Destination
                    </span>
                  )}
            ...
            <div className="mt-3 pt-3 border-t border-zinc-100 dark:border-zinc-800/60 flex flex-col gap-1">
              <div id={`investment-value-${asset.id}`} className="text-xl font-bold tracking-tight text-zinc-900 dark:text-white font-mono">
