...
// We'll replace the existing line
rules = rules.replace(/allow create, update: if isSignedIn\(\) && request\.auth\.uid == userId && \s*\(collectionName != 'assets' \|\| isValidAsset\(request\.resource\.data\)\);/m, collectionRule);
